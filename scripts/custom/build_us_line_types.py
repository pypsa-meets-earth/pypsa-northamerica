...
- ``data/custom/usa/transmission_network/SyntheticUSA.RAW``:
...
- ``data/custom/usa/line_types.csv``:
    Representative US transmission line types.
...
RAW_PATH = Path("data/custom/usa/transmission_network/SyntheticUSA.RAW")
OUTPUT_PATH = Path("data/custom/usa/transmission_network/line_types.csv")
...
def write_line_types(line_types: pd.DataFrame, output_path: Path) -> None:
    """
    Write representative line types to CSV.

    Parameters
    ----------
    line_types : pandas.DataFrame
        Representative line type table.
