...
This custom US version extends alternative clustering to support predefined
US region shapes such as Balancing Authorities, EIA Market Module Regions,
and NEEDS Grid Regions.
...
def busmap_for_us_shape_clusters(n, shape_name, geo_crs, metric_crs):
    """
    Build a busmap by assigning each bus to the nearest predefined US region shape.
    """
    shapes = gpd.read_file(ALTERNATIVE_CLUSTERING_SHAPES[shape_name]).to_crs(geo_crs)
    ...
    joined = gpd.sjoin_nearest(
        buses.to_crs(metric_crs),
        shapes.to_crs(metric_crs),
        how="left",
    )
    joined = joined[~joined.index.duplicated()]

    busmap = joined["index_right"].astype(str) + "_" + buses["carrier"].astype(str)
    busmap.name = "busmap"
...
def clustering_for_n_clusters(
    ...
    if not isinstance(custom_busmap, pd.Series):
        if alternative_clustering:
            alternative_shape = config["cluster_options"].get(
                "alternative_clustering_shape",
                "gadm",
            )
            ...
            if alternative_shape == "gadm":
                ...
                busmap = busmap_for_us_shape_clusters(
                    n,
                    alternative_shape,
                    geo_crs,
                    metric_crs,
